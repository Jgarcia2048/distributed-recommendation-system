...
class Train_Base:
    ...
    def train(self):
        if psutil.virtual_memory().percent < 50 and psutil.cpu_percent() < 50:
            print("Sufficient Hardware Resources For Training...")
            with tqdm(total=100, desc='cpu%', position=1) as cpubar, tqdm(total=100, desc='ram%', position=0) as rambar, tqdm(initial=0, total = self.argv.num_epochs) as pbar:
                ...
                for epoch in range(0, self.argv.num_epochs, 1):
                    ...
                    pbar.update(1)
                    # rs.update(psutil.cpu_percent())
                    if torch.distributed.get_rank() == 0:
                        str_record = "Master Node - epoch = %d, loss = %.3f"%(epoch + 1, loss_epoch_avg)

                    else:
                        str_record = "Slave Node - epoch = %d, loss = %.3f"%(epoch + 1, loss_epoch_avg)
